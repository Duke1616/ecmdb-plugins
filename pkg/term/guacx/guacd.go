...
func (t *Tunnel) Read() (p []byte, err error) {
	...
	if bytes.Equal(data, []byte("rate=44100,channels=2;")) || bytes.Equal(data, []byte("rate=22050,channels=2;")) {
		return nil, nil
	}

	if bytes.Equal(data, []byte("5.audio,1.1,31.audio/L16;")) {
		return append(data, []byte("rate=44100,channels=2;")...), nil
	}
